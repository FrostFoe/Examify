...
export default function QuestionSelector({ selectedIds, onChange, minimal = false }: QuestionSelectorProps) {
    ...
    async function loadFiles() {
      try {
        ...
        if (result.success && result.data) {
          setFiles((Array.isArray(result.data) ? result.data : []) as FileRecord[]);
  ...
  const filteredFiles = files.filter((f) =>
    (f.display_name || f.original_filename).toLowerCase().includes(searchTerm.toLowerCase())
  );

  const filteredQuestions = questions.filter((q) => {
    const matchesSearch = (q.question_text || q.question || "").toLowerCase().includes(searchTerm.toLowerCase());
    ...
    return matchesSearch && matchesSubject && matchesPaper && matchesChapter && matchesHighlight;
  });
    ...
    <div className={cn(
      ...
      <div className={cn("flex flex-col", minimal ? "gap-2" : "gap-3")}>
        <div className="flex items-center justify-between">
            <h3 className="text-xs md:text-sm font-bold flex items-center gap-2">
                {selectedFileId ? (
                    <Button variant="ghost" size="sm" className="h-8 px-2 -ml-2 rounded-lg hover:bg-primary/10 text-primary" onClick={() => {
        ...
        {selectedFileId && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-1">
            ...
            <Select value={filters.paper} onValueChange={(v) => setFilters(f => ({ ...f, paper: v }))}>
              <SelectTrigger className="h-8 text-[10px] rounded-lg">
            ...
            <Select value={filters.chapter} onValueChange={(v) => setFilters(f => ({ ...f, chapter: v }))}>
              <SelectTrigger className="h-8 text-[10px] rounded-lg">
            ...
            <Select value={filters.highlight} onValueChange={(v) => setFilters(f => ({ ...f, highlight: v }))}>
              <SelectTrigger className="h-8 text-[10px] rounded-lg">
        ...
        {loadingFiles || loadingQuestions ? (
          ...
          <div className="space-y-1">
            {filteredFiles.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="truncate max-w-[180px] md:max-w-[220px]">
                        <p className="text-sm font-semibold truncate">{file.display_name || file.original_filename}</p>
                        <p className="text-[10px] text-muted-foreground mt-0.5">{file.total_questions} টি প্রশ্ন</p>
                    </div>
                  </div>
              ...
              <p className="text-center text-muted-foreground py-12 text-sm">কোনো ফাইল পাওয়া যায়নি</p>
            )}
          </div>
        ) : (
          ...
          <div className="space-y-2">
            <div className="bg-primary/5 p-2 rounded-xl mb-3 flex items-center justify-between border border-primary/10 sticky top-0 z-10 backdrop-blur-sm">
                ...
                <Button size="sm" variant="ghost" className="h-7 text-[10px] rounded-lg font-bold hover:bg-primary/10 text-primary" onClick={() => {
                    const allQIds = filteredQuestions.map(q => q.id || "");
                    const allSelected = allQIds.every(id => selectedIds.includes(id));
                    if (allSelected) {
            ...
            {filteredQuestions.length > 0 ? (
                ...
                return (
                  <div 
                    ...
                    className={cn(
                        ...
                        isSelected ? "bg-primary/5 border-primary/30 ring-1 ring-primary/5" : "bg-background border-border hover:border-primary/20"
                    )}
                  ...
                  >
                    <div className="mt-0.5 shrink-0">
                        <div className={cn(
                            ...
                            isSelected ? "bg-primary border-primary text-white scale-110 shadow-sm" : "border-muted-foreground/20 bg-background"
                        )}>
                            {isSelected && <CheckCircle2 className="h-3.5 w-3.5" strokeWidth={3} />}
                        </div>
                    </div>
                    <div className="flex-1 min-w-0 space-y-1.5">
                      <div className="text-xs md:text-sm leading-relaxed font-medium break-words">
                      ...
                      <div className="text-[9px] text-muted-foreground flex items-center gap-2 flex-wrap">
                         <span className="bg-muted px-1.5 py-0.5 rounded">ID: {q.id?.substring(0, 6)}</span>
                         {q.subject && <Badge variant="outline" className="text-[8px] h-3.5 px-1 bg-blue-50 text-blue-600 border-blue-200 font-normal">{q.subject}</Badge>}
                         {q.paper && <Badge variant="outline" className="text-[8px] h-3.5 px-1 bg-green-50 text-green-600 border-green-200 font-normal">{q.paper}</Badge>}
                         {q.chapter && <Badge variant="outline" className="text-[8px] h-3.5 px-1 bg-purple-50 text-purple-600 border-purple-200 font-normal">{q.chapter}</Badge>}
                         {q.highlight && <Badge variant="outline" className="text-[8px] h-3.5 px-1 bg-amber-50 text-amber-600 border-amber-200 font-normal">{q.highlight}</Badge>}
                      </div>
                    </div>
                  </div>
                );
              ...
              <p className="text-center text-muted-foreground py-12 text-sm">এই ফাইলে কোনো প্রশ্ন পাওয়া যায়নি</p>
            )}
          </div>
        )}
      </ScrollArea>
      ...
      {selectedFileId && (
        <div className="pt-2 border-t flex justify-between items-center gap-2">
            <p className="text-[9px] text-muted-foreground italic leading-none">টিপস: প্রশ্নের ওপর ট্যাপ করে সিলেক্ট করুন</p>
            <Button variant="link" size="sm" className="h-auto p-0 text-[10px] font-bold text-primary" onClick={() => setSelectedFileId(null)}>
                ফাইল পরিবর্তন
            </Button>
        </div>
      )}
